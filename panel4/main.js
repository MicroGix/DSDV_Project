...
d3.csv("main_data.csv")
  .then(function (data) {
    ...
    var pieData = Object.entries(counts).map(
      function ([customGroup, customCount]) {
...
d3.csv("main_data.csv")
  .then(function (data) {
    ...
    filteredData.forEach(function (d) {
      ...
      totals[group] = (totals[group] || 0) + parseFloat(d["math score"]) +
        parseFloat(d["reading score"]) + parseFloat(d["writing score"]);
    ...
    var y = d3.scaleBand().range([height, 0]).domain(barData.map(function (d) {
      return d.group;
    })).padding(0.1);
...
d3.csv("main_data.csv")
  .then(function (data) {
    ...
    data.forEach(function (d) {
      d.totalScore = parseFloat(d["math score"]) +
        parseFloat(d["reading score"]) + parseFloat(d["writing score"]);
    });
