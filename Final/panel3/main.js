...
d3.csv(df_url, rowConverter).then(function (data) {
  initPanel_3(data);
});
...
function initPanel_3(data) {
  ...
  const xStack = d3
    ...
    .range([0, w])
    .padding([0.2]);
  const yStack = d3.scaleLinear().domain([0, 1000]).range([h, 0]);
  const colorStack = d3
    .scaleOrdinal()
    .domain(subgroups)
    .range(["#e41a1c", "#377eb8"]);
  ...
  stack.append("g").call(d3.axisLeft(yStack)).attr("class", "chart-axis");
  ...
  // create legend
  createLegend("#stack-plot", d3.map(data, (d) => d.gender).keys(), (d) =>
    colorStack(d)
  );
  ...
  const groupData = Object.entries(TPCbyGroup).map(([group, groupObj]) => {
    const counts = Object.entries(groupObj).map(([tpc, tpcObj]) => {
      // don't understand why here we must use Object.entries instead of Object.values
    ...
    let result = Object.assign({ groups: group }, Object.fromEntries(countTPC));
    return result;
  });
  ...
  const y0Bar = d3
    ...
    .paddingInner(0.2);
  const xBar = d3.scaleLinear().domain([0, 250]).range([0, w]);
  ...
  bar
    ...
    .attr("class", "chart-axis");
  bar.append("g").call(d3.axisLeft(y0Bar)).attr("class", "chart-axis");
  // tooltip
  ...
  let bar_mouseover = function (d) {
    ...
    bar.selectAll("rect").transition().duration(700).style("opacity", 0.5);
    d3.select(this).transition().duration(700).style("opacity", 1);
  };
  ...
  let bar_mouseleave = function (d) {
    ...
    bar.selectAll("rect").transition().duration(700).style("opacity", 1);
    d3.select(this).transition().duration(700).style("opacity", 1);
  };
  bar
    .selectAll(".barchart")
  ...
  // legend
  createLegend("#bar-plot", d3.map(data, (d) => d.tpc).keys(), (d) =>
    colorBar(d)
  );
  ...
  const fnAvg = average(
    data.filter((d) => d.tpc === "none" && d.gender === "female")
  );
  const fcAvg = average(
    data.filter((d) => d.tpc === "completed" && d.gender === "female")
  );
  const fmAvg = average(data.filter((d) => d.gender === "female"));
  // male: none, completed
  const mnAvg = average(
    data.filter((d) => d.tpc === "none" && d.gender === "male")
  );
  const mcAvg = average(
    data.filter((d) => d.tpc === "completed" && d.gender === "male")
  );
